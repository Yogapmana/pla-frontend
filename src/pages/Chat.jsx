...
// Async chat jobs: POST /chat/message returns 202 + job_id and the
// reply is generated by a Celery worker. We poll GET /chat/job/{id}
// until done/failed, with a 5-minute deadline.
const CHAT_JOB_POLL_INTERVAL_MS = 4000
const CHAT_JOB_POLL_TIMEOUT_MS = 5 * 60_000
...
export default function Chat() {
  ...
  // Chat jobs still generating on the backend (202 → poll /chat/job/{id}).
  const [pendingJobs, setPendingJobs] = useState([])
  // Chat jobs that ended in failure — rendered as an error bubble + retry.
  ...
  const sendMutation = useMutation({
    ...
    onSuccess: (data, variables, context) => {
      ...
      // Track it as pending and poll /chat/job/{job_id} until done —
      // the assistant message is saved by the worker, then the
      // history refetch below surfaces it.
      if (data?.pending) {
        setPendingJobs((jobs) => [
          ...jobs,
          {
  ...
  // Poll async chat jobs until done/failed (5-min deadline). Done →
  // invalidate history so the assistant reply (persisted by the
  // worker) appears; failed/timeout → error bubble with retry.
  useEffect(() => {
    ...
    const poll = (job) => {
      ...
      if (Date.now() - job.startedAt > CHAT_JOB_POLL_TIMEOUT_MS) {
        finish(() =>
          setFailedJobs((f) => [
            ...f,
            {
      ...
      getChatJob(job.jobId)
        .then((status) => {
          if (cancelled) return
          if (status.status === 'done') {
            finish(() => {
              queryClient.invalidateQueries({
                queryKey: ['chat', 'general', job.sessionId],
              })
              queryClient.invalidateQueries({ queryKey: ['chat-sessions'] })
            })
            ...
            finish(() =>
              setFailedJobs((f) => [
                ...f,
                {
                  ...
                  sessionId: job.sessionId,
                  content: job.content,
                  reason: status.error || 'failed',
                },
              ])
            )
          } else {
            timers.push(setTimeout(() => poll(job), CHAT_JOB_POLL_INTERVAL_MS))
          }
        })
        .catch(() => {
          // Transient poll error (job record not yet written, network
          // blip, ...) — keep polling until the deadline.
          timers.push(setTimeout(() => poll(job), CHAT_JOB_POLL_INTERVAL_MS))
        })
    }

    pendingJobs.forEach(poll)
    return () => {
      cancelled = true
  ...
  const retryFailedJob = (job) => {
    setFailedJobs((f) => f.filter((j) => j.jobId !== job.jobId))
    if (job.sessionId) {
      sendMutation.mutate({ sessionId: job.sessionId, content: job.content })
    }
  }
  ...
  }, [history, sendMutation.isPending, pendingJobs.length])
            ...
            <AnimatePresence mode="wait">
              {isEmpty ? (
                ...
                >
                  ...
                  {(sendMutation.isPending || pendingJobs.length > 0) && (
                  ...
                  {failedJobs.map((job) => (
                    ...
                    >
                      <div className="mb-5 flex items-start gap-3">
                        ...
                        <div className="flex flex-col gap-2 rounded-2xl rounded-tl-md border border-danger/25 bg-danger/[0.06] px-4 py-3 shadow-warm-xs">
                          <span className="text-xs font-medium text-danger">
                            {t(
                              'chat.job_failed',
                              'Maaf, jawaban gagal dibuat. Silakan coba lagi.'
                            )}
                          </span>
                          <button
                            type="button"
                            onClick={() => retryFailedJob(job)}
                            className="self-start rounded-lg border border-tertiary/25 bg-surface px-3 py-1.5 font-label text-[12px] font-semibold text-tertiary transition-all hover:bg-tertiary/10"
                          >
                            {t('chat.retry', 'Coba lagi')}
                          </button>
                        </div>
                      </div>
                    </motion.div>
