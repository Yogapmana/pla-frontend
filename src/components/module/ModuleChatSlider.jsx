...
import { sendMessage, getChatJob, getHistory } from '@/api/chat'
...
export default function ModuleChatPanel({ sessionId, topicId, moduleTitle }) {
  ...
  // Chat jobs that ended in failure — error bubble + retry.
  ...
  const sendMutation = useMutation({
    ...
    onSuccess: (data, newMessage) => {
      ...
      // Track it as pending and poll /chat/job/{id} until done, then
      // refetch history to surface the persisted reply.
      if (data?.pending) {
        setPendingJobs((jobs) => [
          ...jobs,
          {
  ...
  // invalidate history; failed/timeout → error bubble with retry.
  useEffect(() => {
    ...
    const poll = (job) => {
      ...
      if (Date.now() - job.startedAt > CHAT_JOB_POLL_TIMEOUT_MS) {
        finish(() =>
          setFailedJobs((f) => [
            ...f,
            { jobId: job.jobId, content: job.content, reason: 'timeout' },
          ])
        )
        return
      }
      ...
      <div ref={scrollRef} className="min-h-0 flex-1 overflow-y-auto bg-neutral/40 px-4 py-4 space-y-5">
        {isLoading ? (
          ...
          <div className="space-y-5">
            ...
            {failedJobs.map((job) => (
              <div key={job.jobId} className="flex flex-col gap-2 rounded-2xl rounded-tl-sm border border-danger/25 bg-danger/[0.06] px-4 py-3">
                <span className="text-xs font-medium text-danger">
                  {t(
                    'module.tutor_job_failed',
                ...
                >
                  {t('module.tutor_retry', 'Coba lagi')}
                </button>
              </div>
            ))}
          </div>
